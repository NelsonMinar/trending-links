...
async def test_build_algorithm_and_output(mock_db, test_page_html, tmpdir):
    ...
    async with respx.mock:
        ...
        shutil.copy(os.path.join(real_path, "servers.txt"), os.path.join(tmp_path, "servers.txt"))
        ...
        assert os.path.exists(os.path.join(tmp_path, "output", "trending-links.json"))
        assert os.path.exists(os.path.join(tmp_path, "output", "trending-links.xml"))
        assert os.path.exists(os.path.join(tmp_path, "output", "trending-links.html"))

        # 3. Verify JSON contents and ranking algorithm
        with open(os.path.join(tmp_path, "output", "trending-links.json"), "r") as f:
            feed = json.load(f)
