...
* Fix SQL Injection Vulnerability: In fetch.py, you construct the INSERT query by concatenating Python strings (str(tuple(linkMeta.values()))). If a URL or preview text contains a single quote ('), it will break the query and crash the script. We should use parameterized queries (e.g., cur.execute("INSERT ... VALUES (?, ?, ...)", values)) instead.
* Better Error Handling: build.py uses a bare except: block when fetching previews, which swallows all errors (like KeyboardInterrupts) and makes debugging hard. Also, if a site returns a timeout, it might hang for a while.
* Fix Server Duplication: Remove the duplicate sfba.social in fetch.py.
